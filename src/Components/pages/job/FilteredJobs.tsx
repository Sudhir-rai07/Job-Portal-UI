...
const FilterdJobs = () => {
  ...
  const handleSubmit = async (e:  React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    // setSearchParams({ location, keyword })
    console.log(JobType)
    console.log(searchParams)
    history(`/filter-jobs?location=${location}&keyword=${keyword}&job-type=${JobType}`)
  }
    ...
    <div className="w-full h-screen">
      <div className="w-full h-full overflow-y-scroll">
        ...
        {AllJobs && AllJobs.map((job, idx): JSX.Element =>{
          return (
            <JobCard key={idx} location={job.location} company={job.company} employment_type
            ={job.employment_type
